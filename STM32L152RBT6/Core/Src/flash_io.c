 ...
 * @file     spi_flash.c
 * @version  V1.0.1
 * @brief    W25Q16 SPI Flash 驱动层——读写擦除及应用程序级存储接口
 ************************************************************************************
 * @description
 *
 *  基于 STM32L152RBT6 HAL 库，操作 W25Q16 (2MB) SPI NOR Flash。
 *  移植自官方标准固件库示例，增加应用层参数存取（心跳时间、MAC、4G保活时间）。
 *
 *  硬件连接:
 ...
 *    共占用: 2(帧头) + sizeof(数据结构) + 4(CRC32) = 实际数据长度 + 6
 ************************************************************************************
***/
#include <stdio.h>
#include <string.h>
...
/* ============================================================
 *  应用相关宏定义（#ifndef 守护，允许 user.h 等头文件预定义覆盖）
 *  以下为默认值，用户应根据实际 Flash 分区表修改地址值
 * ============================================================ */
...
#define FLASH_4G_KEEPTIME_ADDR      ((uint32_t)0x010000) /* TODO: 按实际分区填写4G保活时间存储地址 */
#endif
...
extern struct HEART_TIME G_Heart_Time;        /* 心跳时间参数结构体         */
extern uint8_t  G_MAC_Addr[];                 /* MAC 地址缓存              */
extern uint32_t G_4G_KeepConnect_Time;        /* 4G 模块保活连接时间        */
...
/* ============================================================
 *  MISO 引脚模式切换（与其它 SPI 从设备共享 MISO 时使用）
 * ============================================================ */

/**
  * @brief  释放 MISO 引脚为模拟模式
  * @note   将 PA6 设为模拟输入模式，避免多 SPI 从设备共享 MISO 时的总线冲突，
...
/**
  ...
  * @note   将 PA6 设为 AF5 (SPI1_MISO) 复用推挽模式，准备 Flash 通信。
  *         配合 sFLASH_CS_LOW() 在每次 SPI 操作开始前调用。
  * @param  None
  * @retval None
  */
void spi_MISO_Init()
...
/**
  * @brief  初始化 SPI Flash 外设（CS 拉高反选 Flash）
  * @param  None
  * @retval None
  */
...
/**
  * @brief  轮询等待 Flash 忙状态结束（简化版）
  *         - 发送读状态寄存器1命令 (RDSR, 0x05)
  *         - 循环读取并检查 WIP 位 (bit0)，直到为0
  * @note   不使用 static 变量——每次调用都重新初始化并进入循环，
  *         确保每次都实际读取硬件状态寄存器。
  *         与 sFLASH_WaitForWriteEnd() 功能等价，本函数更简洁。
  * @param  None
  * @retval None
  */
...
/**
  * @brief  擦除指定扇区 (4KB)
  * @note   擦除前自动发送写使能 (WREN)，擦除后轮询等待完成。
  *         擦除操作将所有位恢复为 0xFF。
  *         W25Q16 典型扇区擦除时间: ~45ms (max 400ms)
  * @param  SectorAddr: 要擦除的扇区起始地址（24位，须扇区对齐）
  * @retval None
  */
...
/**
  ...
  *          如需真正的一次性全片擦除，请将命令改为 0xC7。
  *          当前函数名保留 Bulk 以兼容历史代码，但行为是擦除单个 64KB 块。
  * @param  None
  * @retval None
  */
...
/**
  * @brief  应用程序级全范围扇区擦除
  * @note   从 FLASH_HEART_TIME_ADDR 开始，以 FLASH_SECTOR_SIZE 为步长，
  *         逐个扇区擦除到 FLASH_4G_KEEPTIME_ADDR。
  *         适用于用户参数存储区域的整体初始化。
  * @param  None
  * @retval None
  */
...
/**
  ...
  * @note   写/擦除操作前必须调用，Flash 内部会将 WEL 位置1。
  *         写完一页或擦完一个扇区后 WEL 自动清零，下次写/擦前需重新使能。
  * @param  None
  * @retval None
  */
...
/**
  * @brief  轮询等待写/擦除操作完成
  * @note   循环发送 RDSR 命令并检查 WIP 位 (bit0)，
  *         为1则继续等待，为0则操作完成。
  * @param  None
  * @retval None
  */
...
/**
  * @brief  缓冲写入——可跨页写入任意长度数据
  * @note   自动处理写入地址非页对齐和跨页拆分：
  *         1. 如果起始地址非页对齐，先写当前页的剩余部分
  *         2. 中间写若干整页
  *         3. 最后写剩余不足一页的部分
  *         这是应用程序最常用的写入接口。
  * @param  pBuffer: 源数据缓冲区指针
  * @param  WriteAddr: Flash 内部目标地址 (24位)
  * @param  NumByteToWrite: 写入总字节数（可任意长度）
  * @retval None
  */
...
{
  /* 读时序: CS低 → READ(0x03) → 24位地址 → 读数据... → CS高 */
  sFLASH_CS_LOW();
...
/**
  * @brief  启动一次连续读序列——发送 READ 命令 + 3字节地址后保持 CS 低
  * @note   与 sFLASH_ReadBuffer() 不同，本函数发送完命令和地址后 CS 保持低电平。
  *         之后调用 sFLASH_ReadByte() 可逐字节连续读取 Flash 数据，
...
void sFLASH_StartReadSequence(uint32_t ReadAddr)
{
  ...
  /* CS 保持低——由调用者后续调用 sFLASH_ReadByte() 连续读取，最后手动 sFLASH_CS_HIGH() */
}

/**
  * @brief  从连续读序列中读取一个字节
  * @note   必须在 sFLASH_StartReadSequence() 之后调用。
  *         每次调用读取一个字节，Flash 内部地址自动+1。
  * @param  None
  * @retval 从 Flash 读到的字节
  */
...
/**
  * @brief  通过 SPI 收发一个字节
  * @note   调用 HAL_SPI_TransmitReceive 实现全双工收发：
  *         发送参数 byte 的同时，接收 Flash 返回的数据。
  *         超时时间 100ms。
  * @param  byte: 要发送的字节
  * @retval 接收到的字节
  */
...
/**
  * @brief  CRC32 计算（多项式 0xEDB88320，与 zlib/gzip 兼容）
 ...
 *  共占用: 2(帧头) + sizeof(DataStruct) + 4(CRC32) = 实际数据长度 + 6
 *  读取时: 校验帧头 + CRC32，通过则拷贝到全局变量，失败则置错误码
 * ============================================================ */

/**
  * @brief  保存任意数据到指定扇区（封装: 先擦除再写）
  * @note   先擦除 SectorAddr 所在扇区，再写入数据。
  *         适用于数据长度不超过一个扇区 (4KB) 的场景。
