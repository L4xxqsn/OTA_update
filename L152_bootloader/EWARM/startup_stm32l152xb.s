...
hal_app_jump:
        //CPSID   I                       ; 关全局中断
        LDR     R1, =0xE000ED08         ; R1 = VTOR 寄存器地址（内核外设）
        STR     R0, [R1]                ; 将 APP 起始地址写入 VTOR

        MOV     R2, #0x00               ; R2 = 0x00000000
        MSR     CONTROL, R2             ; 写入 CONTROL 寄存器

        LDR     R1, [R0]                ; R1 = 读取 APP 向量表的第 1 个字（初始 SP 值）
        MOV     SP, R1                  ; 将 MSP 更新为 APP 的初始栈顶地址

        LDR     R0, [R0, #4]            ; R0 = 读取向量表偏移 4 字节处的值（Reset_Handler 地址）
        BX      R0                      ; 跳转到该地址，并切换指令集（ARM/Thumb）
